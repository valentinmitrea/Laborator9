		...
		public void onClick(View view) {
			if (VoiceCallActivity.instance.ngnSipService != null) {
				...
				NgnMessagingSession instantMessagingSession = NgnMessagingSession.createOutgoingSession(
					VoiceCallActivity.instance.ngnSipService.getSipStack(), remotePartyUri);
				
				if (!instantMessagingSession.sendTextMessage(messageEditText.getText().toString()))
					Log.e(Constants.TAG, "Failed to send message!");
				else {
					conversationTextView.setText(conversationTextView.getText().toString() + "Me: " + messageEditText.getText().toString() + "\n");
